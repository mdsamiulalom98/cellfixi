...
@section('title', 'Company Profile Manage')
                ...
                <div class="page-title-box">
                    <div class="page-title-right">
                        <a href="{{ route('profile.create') }}"
                            class="btn btn-primary waves-effect waves-light btn-sm rounded-pill">Create</a>
                    </div>
                    <h4 class="page-title">Company Profile Manage </h4>
                </div>
            </div>
        </div>
        <!-- end page title -->
                            ...
                            <tbody>
                                @foreach ($show_data as $key => $value)
                                    <tr>
                                        ...
                                        <td>
                                        @if ($value->pdf)
                                            <iframe src="{{asset($value->pdf) }}" width="100%" height="200"></iframe>
                                        ...
                                        <td>
                                            <div class="button-list">
                                                @if ($value->status == 1)
                                                    <form method="post" action="{{ route('profile.inactive') }}"
                                                        class="d-inline">
                                                        @csrf
                                                ...
                                                @else
                                                    <form method="post" action="{{ route('profile.active') }}"
                                                        class="d-inline">
                                                        @csrf
                                                        <input type="hidden" value="{{ $value->id }}" name="hidden_id">
                                                        <button type="button"
                                                ...
                                                <a href="{{ route('profile.edit', $value->id) }}"
